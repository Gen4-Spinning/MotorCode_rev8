...
uint8_t printSettings(void){
	...
	while(1){
		if (firstTime){
			...
			printf("\r\n DC_Voltage:%4.2f, MotorTempC:%d,MosfetTempC:%d",R.voltageVolts,R.MOTtemp,R.FETtemp);
			printf("\r\n ErrorState:%d, ErrorReason:%d",E.errorFlag,R.motorError);
			printf("\r\n Enter -1 to exit");
			firstTime = 0;
		}
			...
			if (C.runningCL == 0){
				if (C.CL_queryNo == 0){
					if (noEntered == -1){
						...
						RM.rampTarget = noEntered;
						printf("\r\n ---- 2.Enter steadyState Time in sec (10-2000)");
						printf("\r\n press -1 to go back");
						C.CL_queryNo = 1;
					}else{
					...
					if (noEntered == -1){
						firstTime = 1;
					}else if ((noEntered >= 10) && (noEntered <= 2000)){
						RM.rampSteadyRunTime_s = noEntered;
						RM.rampRampUpTime_s = RM.rampTarget/50;
						if (RM.rampRampUpTime_s  < 5){
							RM.rampRampUpTime_s  = 5;
						...
						printf("\r\n Not in Range (10-2000). Try Again!");
					}
...
uint8_t Check_CAN_Connection(void){
	int noEntered = 0;
	char skipScanf = 0;
	char firstTime = 1;
	while(1){
		if (firstTime == 1){
			printf("\r\n***MOTOR CAN CONNECTION CHK****");
			printf("\r\n Press 1 to send a Frame to the Motherboard");
			printf("\r\n press -1 to go back");
			skipScanf = 0;
			firstTime = 0;
			}
			...
			if (noEntered == 1){
				printf("\r\n Sent CHK Frame to MotherBoard");
				C.canCHKRecieved = 0;
				T.tim16_oneSecTimer = 0;
				FDCAN_send_CANchk_Frame();
				//wait one sec and then if you havent recieved a response print an error condition
				//canChk recieved will be 1 if you get a response back
				while ((T.tim16_oneSecTimer < 2) && (C.canCHKRecieved == 0)){};
				if (C.canCHKRecieved == 1){
					printf("\r\n Recieved Response From MotherBoard.CAN OK!");
				}else{
					printf("\r\n No response From MotherBoard.Check CAN Connection!");
				}
				printf("\r\n Press 1 to send again, -1 to go back!");
				noEntered = 0;
			}
			if (noEntered == -1){
				break;
...
void configurationFromTerminal(void)
{
	  ...
	  while(1){
		  if (firstTime == 1){
			  ...
			  printf("\r\n 11. Check CAN Connection");
			  printf("\r\n Enter a no btw 1-11 and enter to select an option");
		  ...
		  if (noEntered == 11){
			  firstTime = Check_CAN_Connection();
			  noEntered = 0;
		  }
	  }
}
